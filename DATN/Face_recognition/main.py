    ...
    while stop_ == False:
        ...
        if has_request == True:
            ...
            while not Que.empty():
                ...
                url = f'https://quanlynhansuhaui.000webhostapp.com/php/diemdanh.php'
                _ID = {'id': ids[i]}
                #print(url)
...
def main():    
    ...
    while True:
        ...
        bboxs = detector.findFaces(img, draw=True, Text=False)
        ...
        for(x,y,w,h) in faces:
            ...
            if conf >=percent:
                repeat[id_] = repeat[id_] + 1                 
                #name = labels[id_] + ' - ' + str(round(conf)) +'%'
                #cv2.putText(img, str(name), (x,y-10), font, 0.7, color, stroke, cv2.LINE_AA)            
            #else:
                #name = 'Unknown'
                #cv2.putText(img, str(name), (x,y-10), font, 0.7, color, stroke, cv2.LINE_AA)
            if repeat[id_] >= fps*2:
